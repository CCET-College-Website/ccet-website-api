...
function createTablesIfNotExist($conn) {
    $createIIRC = "CREATE TABLE IF NOT EXISTS `iirc` (
    ...
    if (!$conn->query($createIIRC)) {
        return ["success" => false, "error" => "Failed to create iirc table: " . $conn->error];
    }

    $conn->query("CREATE INDEX IF NOT EXISTS idx_iirc_section ON `iirc`(section)");
    $conn->query("CREATE INDEX IF NOT EXISTS idx_iirc_is_active ON `iirc`(is_active)");
    $conn->query("CREATE INDEX IF NOT EXISTS idx_iirc_order ON `iirc`(display_order)");
    ...
    case 'POST':
        ...
        if ($conn->query($sql)) {
            ...
            echo json_encode([
                ...
                "title" => $input['title'],
                "message" => "IIRC item added successfully"
            ]);
        } else {
            echo json_encode(["success" => false, "error" => $conn->error]);
        }
