...
function createTablesIfNotExist($conn) {
    $createIPRC = "CREATE TABLE IF NOT EXISTS `iprc` (
    ...
    if (!$conn->query($createIPRC)) {
        return ["success" => false, "error" => "Failed to create iprc table: " . $conn->error];
    }

    $conn->query("CREATE INDEX IF NOT EXISTS idx_iprc_section ON `iprc`(section)");
    $conn->query("CREATE INDEX IF NOT EXISTS idx_iprc_is_active ON `iprc`(is_active)");
    $conn->query("CREATE INDEX IF NOT EXISTS idx_iprc_order ON `iprc`(display_order)");
    ...
    case 'POST':
        ...
        if ($conn->query($sql)) {
            ...
            echo json_encode([
                ...
                "title" => $input['title'],
                "message" => "IPRC item added successfully"
            ]);
        } else {
            echo json_encode(["success" => false, "error" => $conn->error]);
        }
