...
function createTablesIfNotExist($conn) {
    ...
    if (!$conn->query($createHistory)) {
        return ["success" => false, "error" => "Failed to create history table: " . $conn->error];
    }

    $conn->query("CREATE INDEX IF NOT EXISTS idx_history_section ON `history`(section)");
    $conn->query("CREATE INDEX IF NOT EXISTS idx_history_is_active ON `history`(is_active)");
    $conn->query("CREATE INDEX IF NOT EXISTS idx_history_order ON `history`(display_order)");
...
switch ($method) {
    ...
    case 'POST':
        ...
        $sql = "INSERT INTO `$tableName` (section, title, description, year, image_url, display_order, is_active) 
                VALUES ('$section', '$title', $description, $year, $image_url, $display_order, $is_active)";
    ...
    case 'PATCH':
        ...
        $allowedFields = ['section', 'title', 'description', 'year', 'image_url', 'display_order', 'is_active'];
