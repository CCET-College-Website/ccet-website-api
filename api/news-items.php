...
function createTablesIfNotExist($conn) {
    // News Items Table
    $createNewsItems = "CREATE TABLE IF NOT EXISTS news_items (
    ...
    if (!$conn->query($createNewsItems)) {
        return ["success" => false, "error" => "Failed to create news_items table: " . $conn->error];
    }

    // Create Indexes
    $conn->query("CREATE INDEX IF NOT EXISTS idx_news_items_date ON news_items(date DESC, is_active)");
    $conn->query("CREATE INDEX IF NOT EXISTS idx_news_items_order ON news_items(display_order, is_active)");
...
switch ($method) {
    case 'GET':
        ...
        if (isset($_GET['keyword'])) {
            ...
            $result = $conn->query("SELECT * FROM news_items WHERE (" . implode(" OR ", $searchConditions) . ") ORDER BY date DESC");
    ...
    case 'POST':
        ...
        $sql = "INSERT INTO news_items (" . implode(", ", $columns) . ") VALUES (" . implode(", ", $values) . ")";
    ...
    case 'PATCH':
        ...
        $sql = "UPDATE news_items SET " . implode(", ", $updates) . " WHERE " . implode(" AND ", $whereClauses);
    ...
    case 'DELETE':
        ...
        $sql = "UPDATE news_items SET is_active = FALSE, updated_at = CURRENT_TIMESTAMP WHERE " . implode(" AND ", $whereClauses);
